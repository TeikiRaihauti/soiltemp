...
parameters_name = set(e.name for e in mu.parameters )
print(f"Parameters : {parameters_name}")


names = ['states', 'rates', 'auxiliary', 'exogenous', 'parameters']  # list of names as strings

for name in names:
    attr = getattr(mu, name)  # get the object from the module
    print('######' + name)
    exos = {}
    for e in attr:
        res = e.name.split('_')
